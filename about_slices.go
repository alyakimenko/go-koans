...
func aboutSlices() {
	...
	assert(cap(pregnancy_slots) == (len(pregnancy_slots)-1)*2) // but with better optimizations
	...
	assert(cap(pregnancy_slots) == (len(pregnancy_slots)-1)*2) // the capacity optimizations have a guessable algorithm
}
